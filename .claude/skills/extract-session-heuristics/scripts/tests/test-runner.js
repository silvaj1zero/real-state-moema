...
test('T2 status returns valid operator state', () => {
  ...
  assertEq(data.operator.slug, 'oalanicolas', 'slug should be oalanicolas');
...
test('T3 scan matches filesystem state', () => {
  ...
  assertEq(data.operator, 'oalanicolas', 'operator should be oalanicolas');
...
test('T4 bootstrap uses persistent_state when file exists', () => {
  ...
  assertEq(data.operator.slug, 'oalanicolas', 'operator should be oalanicolas');
});
...
test('T7 all AN_KE_143-152 pass compliance gate', () => {
...
// ================================================================
// T8 — check-overlap with rich keywords finds AN_KE_143
// ================================================================

test('T8 check-overlap detects existing AN_KE_143 for audit/align keywords', () => {
  ...
  assertContains(ids, 'AN_KE_143', 'AN_KE_143 should be in top matches');
});
...
test('T10 persist dry-run validates batch without side effects', () => {
  ...
  const batch = {
    heuristic_files: [
      path.join(NAMESPACE, 'AN_KE_143.md'),
      path.join(NAMESPACE, 'AN_KE_144.md'),
    ],
    new_cards: [
...
test('T15 active-operator.yaml is gitignored', () => {
  try {
    execSync('git check-ignore .aiox/active-operator.yaml', {
      cwd: REPO_ROOT,
      stdio: 'pipe',
    });
    // exit 0 means it IS ignored
  } catch (err) {
    throw new Error('.aiox/active-operator.yaml should be gitignored (git check-ignore returned non-zero)');
  }
});
...
test('T17 npm run sinkra:operator -- list works', () => {
  try {
    ...
    assertContains(stdout, 'oalanicolas', 'list should mention oalanicolas');
