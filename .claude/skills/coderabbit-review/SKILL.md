...
description: "CodeRabbit CLI execution via WSL with self-healing loop — automated code review before commits, PRs, or QA gates."
version: "1.0.0"
owner_squad: infra-ops-squad
sinkra_tier: Tier1
context: fork
agent: coderabbit-review
user-invocable: true
...
> **Integration note:** This skill is invoked by `/develop-story` (Phase 5, uncommitted scope) and `/review-story` (Step 0, committed scope) via background Agent delegation. It can also be invoked standalone.
...
Detect the project root dynamically, then run CodeRabbit:

```bash
PROJECT_ROOT=$(wsl bash -c "wslpath -a '$(git rev-parse --show-toplevel)'")
wsl bash -c "cd $PROJECT_ROOT && ~/.local/bin/coderabbit {flags}"
```
