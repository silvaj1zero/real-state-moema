---
name: me-guia
description: 'Universal intent router for AIOX — resolves ambiguous requests to existing squads/tasks/agents. Triggers: "me guia", "me ajuda", "preciso de", "quero", "o que eu faço", "como faço".'
---

# Me Guia v1.0

Universal intent router for the AIOX framework. Transforms any user request into an
...
EXISTS in the AIOX registries. When a gap is found, it transparently reports the gap and
...
3. For `domain`: grep keywords against `.aiox/squad-runtime/ecosystem-registry.yaml`
...
2. Match extracted `domain` keywords against (in priority order):
   a. Squad `keywords[]` — base keywords defined by squad author
   b. Squad `routing_synonyms[]` — learned from successful routings (Phase 7)
   c. Squad `capabilities[]` — capability descriptions
   d. Squad `name` and `description` — fallback text matching
3. Apply `routing_weight` as multiplier (1.0-10.0, default 1.0) to match score
4. Rank squads by weighted match score
5. If top squad score > 0.7, select it
6. If top 2-3 squads are close, present disambiguation (Phase 1 MEDIA path)

### Level 2: Task Resolution (within selected squad)

1. Search in order:
   a. `squads/{squad}/tasks/` -- squad-specific tasks
   b. `.aiox-core/development/tasks/` -- core tasks matching squad domain
   c. `.aiox-core/manifests/tasks.csv` -- flat index by name/category
   d. `.aiox-core/data/entity-registry.yaml` -- entity keywords
...
For each pipeline step needed:
  ...
  2. Grep core tasks: .aiox-core/development/tasks/*.md
  3. Grep entity registry: .aiox-core/data/entity-registry.yaml
  4. Grep tasks manifest: .aiox-core/manifests/tasks.csv
```
...
1. **Report transparently**:
   ...
   - [task-b] (.aiox-core/tasks/) -- [por que nao serve]
...
2. **Determine creation target**:
   ...
   - If the task belongs to the **AIOX core** (general dev, infra, framework):
     Target: `@aiox-master` via `.aiox-core/development/tasks/create-task.md`

3. **Ask user confirmation**:
   ```
   Quer que eu crie essa task?
   - Responsavel: @squad-chief (squad: {squad_name}) | @aiox-master (core)
...
4. **If user accepts, generate handoff**:
   ...
   For **core task** (via @aiox-master):
   ```yaml
   handoff:
     to: aiox-master
     task: .aiox-core/development/tasks/create-task.md
     input:
...
- **Handoff:** @squad-chief | @aiox-master
- **Task:** create-task
...
## Phase 6: Resolution Logging (MANDATORY)

After EVERY invocation, append to `outputs/me-guia/resolution-log.yaml`.
This log is the raw data that feeds the learning engine (Phase 7).
Without logging, the feedback loop has no data to learn from.
...
## Phase 7: Closed-Loop Learning (Automatic)

The resolution log (Phase 6) feeds a learning engine that enriches the ecosystem-registry
with routing intelligence. This closes the feedback loop, making me-guia smarter over time.

### How It Works

```
me-guia logs routing ──► resolution-log.yaml ──► *sync-ecosystem Phase 7
                                                         │
                                                         ▼
                                                  me-guia-learn.js
                                                         │
                                         ┌───────────────┼───────────────┐
                                         ▼               ▼               ▼
                                  routing_synonyms  routing_weight  gap_report
                                         │               │               │
                                         ▼               ▼               ▼
                                  ecosystem-registry.yaml enriched
                                         │
                                         ▼
                                  me-guia routes BETTER next time
```

### What It Learns

| Signal | Source | Output | Where Stored |
|--------|--------|--------|--------------|
| Synonyms | User keywords from successful routings | `routing_synonyms[]` per squad | ecosystem-registry.yaml |
| Weights | Match frequency per squad (time-decayed) | `routing_weight` (1.0-10.0) | ecosystem-registry.yaml |
| Gaps | Recurring unresolved capabilities | Gap report | outputs/me-guia/learning-metrics.json |

### Trigger

The learning engine runs automatically as Phase 7 of `*sync-ecosystem`.
It can also run standalone:

```bash
node scripts/me-guia-learn.js           # Full learning cycle
node scripts/me-guia-learn.js --dry-run  # Preview without writing
```

### Learning Rules

1. Only learns from `outcome: "completed"` + `user_confirmed: true` entries
2. Entries older than 30 days get 0.5x weight (decay)
3. Stop words (PT-BR + EN) are excluded from synonyms
4. Max 30 synonyms per squad (prevents noise)
5. Synonyms are additive — never removes existing keywords
6. Weights normalize to 1.0-10.0 scale across all squads

## Registry Files Used

| Registry | Path | Purpose |
|----------|------|---------|
| Ecosystem Registry | `squads/sinkra-squad/data/ecosystem-registry.yaml` | Squad keywords, routing_synonyms, routing_weight, capabilities |
| Entity Registry | `.aiox-core/data/entity-registry.yaml` | 498 entities with purpose, keywords |
| Tasks Manifest | `.aiox-core/manifests/tasks.csv` | 197 tasks with name, category, elicitation flag |
| Squad Tasks | `squads/*/tasks/*.md` | Squad-specific task definitions |
| Core Tasks | `.aiox-core/development/tasks/*.md` | Core framework task definitions |
| Workspace Templates | `workspace/_templates/` | Template availability for input matching |
| Resolution Log | `outputs/me-guia/resolution-log.yaml` | Routing decisions (feeds learning engine) |
| Learning Metrics | `outputs/me-guia/learning-metrics.json` | Learning cycle observability |
...
| Core task creation | @aiox-master | `.aiox-core/development/tasks/create-task.md` | Gap in core domain |
