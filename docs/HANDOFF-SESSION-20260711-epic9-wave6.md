...
| C | 9.4 engine | @data-engineer + @devops | **9.4 fechamento** | ✅ **EXECUTADA 12-Jul — 9.4 Done.** Backup feito (`_backups/acm-imobiliario-backup-2026-07-12.zip`). Sink ampliado no engine local + `backfill_story94.py`: match 3.618/3.618, apply 3.618 UPDATE 0 erros. `9.4-sink-coverage.mjs` EXIT 0 (0% NULL nos campos-meta); adapter do app mapeia `data_venda`/R5/`bairro_real`; smoke in-app: ajustados=68/semAjuste=0, gate R5 780 verticais via guia. Suíte ACM 303 PASS. **D/E/F desbloqueadas** (pré-condição "9.4 Done" satisfeita). Nota: engine tem 194 vendas 2026 novas p/ próximo re-sink (`push_acm_supabase.py --apply`). | **nenhuma** (desbloqueada 12-Jul) |
