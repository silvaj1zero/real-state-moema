# -*- coding: utf-8 -*-
"""PITCH DE CAPTAÇÃO v2 (Rua Alvorada, 116 — apto 52, Ed. Manuela) — .pptx editável.
Design RE/MAX Galeria (mesmo DS do pitch Honduras). 14 slides, 16:9.
Revisão com observações da Luciana (05/08/2026): 2 dormitórios (3º virou closet),
prédio 1995, sem lazer completo (academia nova + playground), reforma jul/2026,
sem destaque para 213 m² (só nota técnica), slide de balança a favor/contra,
anúncio R$ 1.390.000, fechamento R$ 1,25–1,35 mi, concorrente EmCasa Cardoso de Melo."""
...
def checklist(s, x,y,w, items, size=14, gap=9):
    ...
    for i,it in enumerate(items):
        ...
        segs = it if isinstance(it, list) else [(it, CINZA700, None)]
        for seg in segs:
            t,c,bd=(seg+(None,)*3)[:3]
            r=p.add_run(); r.text=t; r.font.size=Pt(size); r.font.name=BODY
            r.font.color.rgb=CINZA700 if c is None else c; r.font.bold=False if bd is None else bd
    return b

def crosslist(s, x,y,w, items, size=14, gap=9):
    b=s.shapes.add_textbox(Inches(x),Inches(y),Inches(w),Inches(5)); tf=b.text_frame
    tf.word_wrap=True; tf.margin_left=0;tf.margin_right=0;tf.margin_top=0;tf.margin_bottom=0
    for i,it in enumerate(items):
        p = tf.paragraphs[0] if i==0 else tf.add_paragraph()
        p.space_after=Pt(gap); p.line_spacing=1.12
        rc=p.add_run(); rc.text="—  "; rc.font.size=Pt(size); rc.font.name=BODY; rc.font.color.rgb=VERM; rc.font.bold=True
...
# S6 SOBRE O IMÓVEL
s=newslide(); kicker_title(s,"Sobre o seu imóvel · características confirmadas","Apto reformado em 2026 num prédio boutique de só 20 unidades",tsize=26)
txt(s,MX,2.0,7.2,1.0,[[("No coração da Vila Olímpia, o ",CINZA700,False),("Ed. Manuela (1995)",TINTA,True),(" tem apenas ",CINZA700,False),("2 apartamentos por andar",TINTA,True),(". O apto 52 fica no ",CINZA700,False),("5º de 10 andares",TINTA,True),(", com a sala de frente para a rua, ",CINZA700,False),("reformado em julho de 2026",TINTA,True),(" e pronto para morar.",CINZA700,False)]],14.5,CINZA700,ls=1.3)
bullets(s,MX,3.1,7.2,[
    [("2 dormitórios (1 suíte)",TINTA,True),(" — o 3º dormitório foi convertido em closet da suíte (reversível) — + lavabo, varanda e dependência completa.",CINZA700,False)],
    [("Reforma de julho/2026:",TINTA,True),(" pintura 100%, registros, vasos sanitários e torneiras novos, piso de madeira restaurado.",CINZA700,False)],
    [("Ventilação cruzada e janelas grandes:",TINTA,True),(" sala de frente, cozinha e lavanderia aos fundos, dormitórios laterais.",CINZA700,False)],
    [("Prédio em dia:",TINTA,True),(" academia nova, playground, elevadores novos, AVCB e alarme de incêndio.",CINZA700,False)],
],size=13,gap=8)
sourcebox(s,MX,5.6,7.2,1.0,[[("Due diligence antes do anúncio: ",TINTA,True),("confirmar o carnê de IPTU do próprio apto 52 e a averbação das 2 vagas na matrícula.",CINZA700,False)]],size=11.5)
xr=8.2; cw2s=(SW-MX-xr-0.25)/2
stat(s,xr,2.0,cw2s,2.05,"113"," m²","Área útil",accent=True)
stat(s,xr+cw2s+0.25,2.0,cw2s,2.05,"2","","Vagas privativas")
stat(s,xr,4.25,cw2s,2.05,"20","","Unidades no prédio (2 por andar)")
stat(s,xr+cw2s+0.25,4.25,cw2s,2.05,"R$ 250"," mil","Caixa do condomínio",numsize=26)
foot(s,6)

# S7 BALANÇA (A FAVOR × O QUE PESA)
s=newslide(); kicker_title(s,"Precificação honesta · os dois lados da balança","O que joga a favor — e o que o comprador vai pesar",tsize=27)
cw2=(CW-0.6)/2
txt(s,MX,2.0,cw2,0.35,"A FAVOR",12,OK,bold=True)
checklist(s,MX,2.4,cw2,[
    [("Reforma completa em julho/2026",TINTA,True),(" — excelente conservação",CINZA700,False)],
    [("Boutique:",TINTA,True),(" 20 unidades, 2 aptos por andar",CINZA700,False)],
    [("Ventilação cruzada e janelas grandes",CINZA700,False)],
    [("Academia nova e playground",CINZA700,False)],
    [("Manutenção em dia: elevadores novos, AVCB, alarme de incêndio",CINZA700,False)],
    [("Caixa do condomínio: ",CINZA700,False),("R$ 250 mil",TINTA,True)],
    [("Localização residencial: metrô Eucaliptos a 800 m, Hosp. Santa Paula a 150 m",CINZA700,False)],
],size=12.5,gap=8)
xr=MX+cw2+0.6
txt(s,xr,2.0,cw2,0.35,"O QUE O COMPRADOR VAI PESAR",12,VERM,bold=True)
crosslist(s,xr,2.4,cw2,[
    "Prédio com mais de 30 anos (1995)",
    "2 dormitórios efetivos (o 3º virou closet)",
    "Sem área de lazer completa",
    "Garagem difícil",
    "Sem ar-condicionado e sem gerador",
    "Condomínio e IPTU altos",
],size=12.5,gap=8)
sp=rect(s,MX,6.0,CW,0.85,AZUL,shape=MSO_SHAPE.ROUNDED_RECTANGLE); sp.adjustments[0]=0.10
txt(s,MX+0.3,6.08,CW-0.6,0.7,[[("É esse balanço que define a faixa realista. ",BRANCO,True),("O comprador vai fazer exatamente esta conta — nós fazemos primeiro e chegamos com o preço certo.",ICE2,False)]],13.5,ICE2,anchor=MSO_ANCHOR.MIDDLE,ls=1.2)
foot(s,7)
...
stat(s,MX+cw3+0.3,y,cw3,hh,"53","","Vendas do perfil exato do alvo — mesmo porte no cadastro municipal e prédios de 1985–2005 (o Manuela é de 1995)")
stat(s,MX+2*(cw3+0.3),y,cw3,hh,"IPTU","","Carnê do próprio edifício ancora o piso de valor (venal R$ 1,10 mi)",accent=True)
txt(s,MX,4.5,CW,0.7,[[("Cada comparável é uma ",CINZA700,False),("transação registrada em cartório",TINTA,True),(", rastreável pelo cadastro municipal (SQL). Preço de anúncio entra só como leitura de concorrência — ",CINZA700,False),("nunca como verdade de valor",TINTA,True),(".",CINZA700,False)]],14.5,CINZA700,ls=1.3)
sourcebox(s,MX,5.3,CW,0.95,[[("A idade do prédio é o fator que mais separa preço: ",TINTA,True),("no mesmo recorte de área, imóveis de todas as épocas têm mediana de R$ 1,95 mi — mas o estoque de 1985–2005 fica em R$ 1,31 mi. Comparar o Ed. Manuela com prédio novo inflaria o valor e travaria a venda.",CINZA700,False)]],size=11.5)
txt(s,MX,6.35,CW,0.5,"Nota técnica: a comparação venda-contra-venda usa a base cadastral do IPTU — a mesma régua para todos os imóveis (no prédio, essa base inclui área comum e as 2 vagas). No anúncio, a área é sempre a útil: 113 m².",10.5,CINZA500,ls=1.2)
foot(s,8)
...
cw3=(CW-0.6)/3; y=2.05; hh=3.15
card(s,MX,y,cw3,hh,accent=True)
txt(s,MX+0.25,y+0.2,cw3-0.5,0.6,"1 · O vizinho de porta",17,BRANCO,bold=True,font=HEAD)
txt(s,MX+0.25,y+0.8,cw3-0.5,2.2,[[("O apto 51 — mesmo prédio, mesmo 5º andar, mesma planta — vendido por ",ICE2,False),("R$ 1.325.000",BRANCO,True),(" (set/2023), em compra e venda registrada. Corrigido pelo micro-mercado: ",ICE2,False),("~R$ 1,31 mi",BRANCO,True),(" hoje. Não existe comparável melhor em uma ACM.",ICE2,False)]],12,ICE2,ls=1.25)
x2=MX+cw3+0.3
card(s,x2,y,cw3,hh)
txt(s,x2+0.25,y+0.2,cw3-0.5,0.6,"2 · O prédio gêmeo",17,AZUL,bold=True,font=HEAD)
txt(s,x2+0.25,y+0.8,cw3-0.5,2.2,[[("Vahia de Abreu, 383 — mesma safra, a 390 m — teve ",CINZA700,False),("10 vendas",TINTA,True),(" em 2024–26, entre ",CINZA700,False),("R$ 1,00 e 1,24 mi",TINTA,True),(". O Manuela vale mais: boutique de 20 unidades, 2 vagas e academia nova.",CINZA700,False)]],12,CINZA700,ls=1.25)
x3=MX+2*(cw3+0.3)
card(s,x3,y,cw3,hh)
txt(s,x3+0.25,y+0.2,cw3-0.5,0.6,"3 · O mercado da região",17,AZUL,bold=True,font=HEAD)
txt(s,x3+0.25,y+0.8,cw3-0.5,2.2,[[("53 vendas reais",TINTA,True),(" do mesmo porte e da mesma safra no raio de 1.000 m — mediana de ",CINZA700,False),("R$ 1,31 mi",TINTA,True),(", batendo com a evidência nº 1.",CINZA700,False)]],12,CINZA700,ls=1.25)
sp=rect(s,MX,5.45,CW,0.8,AZUL,shape=MSO_SHAPE.ROUNDED_RECTANGLE); sp.adjustments[0]=0.10
txt(s,MX+0.3,5.52,CW-0.6,0.65,[[("Convergência: ",BRANCO,True),("as três evidências apontam para a mesma região de valor — o fechamento realista é ",ICE2,False),("R$ 1,25–1,35 mi",BRANCO,True),(".",ICE2,False)]],13.5,ICE2,anchor=MSO_ANCHOR.MIDDLE,ls=1.2)
txt(s,MX,6.4,CW,0.5,"As demais transações do próprio prédio não são referência de mercado (leilão, integralização de capital e uma venda atípica abaixo do valor venal). Se um comprador citar a mais baixa, a resposta está pronta: contra ela há uma venda de mercado no mesmo andar por R$ 1,325 mi.",10.5,CINZA500,ls=1.2)
foot(s,10)

# S11 CONCORRÊNCIA + POSICIONAMENTO
...
txt(s,MX,1.9,CW,0.6,[[("Na régua que o comprador usa (R$/m² útil), o anúncio de ",CINZA700,False),("R$ 1.390.000",TINTA,True),(" posiciona o seu apartamento com folga abaixo dos pedidos inflados da própria rua — e à frente do concorrente mais perigoso:",CINZA700,False)]],13,CINZA700,ls=1.25)
table(s, MX, 2.6, CW,
    ...
    [
        ...
        ["R. Alvorada","109 m²","R$ 1.700.000","R$ 15.596","acima"],
        ["SEU IMÓVEL (anúncio)","113 m²","R$ 1.390.000","R$ 12.301","—"],
        ["R. Casa do Ator","115 m²","R$ 1.280.000","R$ 11.130","abaixo"],
        ["Av. Dr. Cardoso de Melo (4 dorms)","140 m²","R$ 1.490.000","R$ 10.643","maior, preço total próximo"],
        ["R. Alvorada (1 vaga)","110 m²","R$ 1.170.000","R$ 10.636","abaixo"],
    ],
    colw=[3.6,1.3,2.0,1.8,3.4], hl_row=3, fsize=10.5, rowh=0.36)
sourcebox(s,MX,5.75,CW,1.1,[[("O alerta do R$ 1,5 mi: ",TINTA,True),("na Av. Dr. Cardoso de Melo, um apto de 140 m² úteis e 4 dormitórios pede R$ 1.490.000. Qualquer pedido nosso perto de R$ 1,5 mi compete de frente com ele — e perde. Os fechamentos reais equivalentes ficam em R$ 1,20–1,65 mi (gap de 25–30% sobre os anúncios). ",CINZA700,False),("Precificar certo desde o início protege o seu patrimônio.",TINTA,True)]],size=11)
foot(s,11)

# S12 RECOMENDAÇÃO (HERO)
s=newslide(dark=True)
...
txt(s,MX,1.6,7.5,1.1,[[("R$ ",ICE,True),("1.390.000",BRANCO,True)]],54,BRANCO,bold=True,font=HEAD)
txt(s,7.6,1.25,CW-7.0,0.35,"Fechamento esperado",13,ICE2)
txt(s,7.6,1.6,5.5,1.0,[[("R$ ",ICE,True),("1,25–1,35",BRANCO,True),(" mi",ICE,True)]],40,BRANCO,bold=True,font=HEAD)
txt(s,MX,3.0,CW,0.35,"A RÉGUA DE VALOR (R$ MILHÕES)",12,ICE,bold=True)
cw3=(CW-2*0.25)/3; y=3.45; hh=1.35
labels=[("R$ 1,39","Anúncio",False),("R$ 1,25–1,35","Fechamento esperado",True),("R$ 1,10","Venal oficial (IPTU) — piso de defesa",False)]
for i,(v,l,hl) in enumerate(labels):
    x=MX+i*(cw3+0.25)
    sp=rect(s,x,y,cw3,hh, VERM if hl else AZUL700, shape=MSO_SHAPE.ROUNDED_RECTANGLE); sp.adjustments[0]=0.10
    txt(s,x+0.1,y+0.22,cw3-0.2,0.5,v,20,BRANCO,bold=True,font=HEAD,align=PP_ALIGN.CENTER)
    txt(s,x+0.1,y+0.8,cw3-0.2,0.5,l,11,RGBColor(0xff,0xd9,0xdf) if hl else ICE2,align=PP_ALIGN.CENTER,ls=1.1)
rect(s,MX,5.15,0.06,1.6,VERM)
txt(s,MX+0.25,5.2,CW-0.5,1.6,[
    [("A régua é o seu próprio prédio: ",BRANCO,True),("a venda do vizinho de porta corrigida (~R$ 1,31 mi), o balanço honesto de pontos fortes e fracos e a concorrência real.",ICE2,False)],
    [("R$ 1.390.000 entra no filtro de \"até R$ 1,4 mi\" dos portais",BRANCO,True),(", fica abaixo do concorrente maior de R$ 1,49 mi e mantém reserva de negociação até o fechamento esperado. Na negociação, o valor é defendido pelo venal oficial e pela venda do AP 51.",ICE2,False)],
...
steps=[
    ...
    ("4","Implementação e início dos trabalhos — anúncio a R$ 1.390.000, com revisão em ciclos de 45–60 dias.",False),
    ("5","VENDA — fechamento entre R$ 1,25 e R$ 1,35 milhão.",True),
]
y=2.15
...
out = os.path.join(HERE, "PITCH_Captacao_Alvorada116_Dorothy_RE-MAX_v2.pptx")
prs.save(out)
print("OK:", out)
